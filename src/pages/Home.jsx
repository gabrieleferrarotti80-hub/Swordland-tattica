        ...
        <button onClick={() => {
            ...
            <span className="text-2xl font-black text-white">Battaglia Castello</span>
                  ...
                  {hubView === 'alliance' && (
                    <div className="flex flex-col w-full px-2 md:px-8 gap-4 lg:gap-8 animate-in slide-in-from-right-8 duration-300">
                      <button onClick={() => setHubView('main')} className="text-slate-400 hover:text-white font-bold text-xs uppercase mb-2 text-left w-fit flex items-center px-6 py-3 border border-slate-700 bg-slate-900 rounded-full">{t('home.back_menu')}</button>
                      <div className={`grid grid-cols-1 ${auth.allianceRole === 'officer' || auth.role === 'admin' || auth.role === 'consulente' ? 'lg:grid-cols-2 xl:grid-cols-4' : 'max-w-sm mx-auto'} gap-4 lg:gap-8 w-full`}>
             ...
             <div className="flex flex-col w-full h-full bg-slate-900/80 backdrop-blur-2xl p-6 rounded-3xl border border-white/10 overflow-y-auto">
               ...
               <RosterTable roster={roster} onAddPlayer={(p) => setRoster(prev => [...prev, { id: `player-${Date.now()}`, ...p }])} onEdit={(id, field, value) => setRoster(prev => prev.map(pl => pl.id === id ? { ...pl, [field]: value } : pl))} onDelete={(id) => setRoster(prev => prev.filter(pl => pl.id !== id))} onDeploy={() => setIsRosterOpen(false)} />
