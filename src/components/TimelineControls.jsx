...
export const TimelineControls = ({
  ...
  return (
    // Aggiunto h-full per occupare tutto lo spazio verticale del genitore
    <div className="flex flex-col items-center py-4 px-3 h-full justify-between gap-2">
      
      {/* 1. INDICATORE MINUTO (Numeri rimpiccioliti) */}
      <div className="text-center shrink-0">
        ...
        <div className="text-[9px] font-bold uppercase tracking-widest text-slate-500 mt-1">Minuto</div>
      </div>

      {/* 2. SLIDER VERTICALE (Ora si flette automaticamente con flex-1) */}
      ...
      {/* 3. PULSANTIERA AZIONI (Bottoni ridimensionati) */}
      <div className="flex flex-col items-center gap-2 w-full shrink-0">
        {hasDrafts ? (
          ...
          <div className="flex flex-col items-center gap-2 w-full animate-in fade-in duration-200">
            {/* Tasto play più piccolo (w-12 h-12 anziché 14) */}
            <button 
              onClick={togglePlay} 
            ...
            >
              ...
              <option value={15}>15x Vel.</option>
            </select>
          </div>
        )}
      </div>
