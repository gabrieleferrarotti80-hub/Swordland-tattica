...
export default function EventManagerModal({ 
  ...
  const deletePlanFromFirebase = async (id) => {
    if (window.confirm(t('event_manager.confirm_delete', 'Vuoi davvero eliminare questo piano?'))) {
      try {
        ...
        alert(t('event_manager.delete_error', "Errore durante l'eliminazione."));
      }
    }
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (parsed.tacticalMeta || parsed.playerOverrides || parsed.activeDeployment) {
          ...
          alert(t('event_manager.invalid_file', 'File non valido.'));
        }
      } catch (err) {
        alert(t('event_manager.read_error', 'Errore di lettura.'));
      }
    };
  ...
  const formatDate = (timestamp) => {
    if (!timestamp || !timestamp.seconds) return t('event_manager.unknown_date', 'Data Sconosciuta');
        ...
        <div className="flex justify-between items-start mb-6 border-b border-slate-800 pb-4 shrink-0">
          <div>
            <h2 className="text-2xl font-black text-white flex items-center gap-3">
              <span>☁️</span> {t('event_manager.title', 'Database Piani Cloud')}
            </h2>
          ...
          <div className="flex-1 bg-slate-950/50 border border-slate-800 rounded-2xl flex flex-col shadow-inner overflow-hidden">
            <div className="bg-slate-900 p-4 border-b border-slate-800 flex justify-between items-center shrink-0">
              <h3 className="text-sm font-black text-cyan-400 uppercase tracking-widest flex items-center gap-2">
                {t('event_manager.cloud_archive', 'Archivio Cloud')}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-3 min-h-[300px]">
              {isFetching && savedPlans.length === 0 ? (
                <div className="text-center text-cyan-400 font-bold py-10 animate-pulse">{t('event_manager.syncing', 'Sincronizzazione in corso...')}</div>
              ) : savedPlans.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 py-10 animate-in fade-in duration-500">
                  ...
                  <p className="text-sm font-bold">{t('event_manager.no_plans', 'Nessun piano salvato')}</p>
                  <p className="text-xs italic mt-1">{t('event_manager.no_plans_desc', 'Crea o importa un nuovo piano.')}</p>
                </div>
              ) : (
                savedPlans.map(plan => (
                  <div key={plan.id} className={`bg-slate-900 border p-3 rounded-xl flex items-center justify-between hover:border-cyan-500/50 transition-colors group ${plan.id === currentPlanId ? 'border-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'border-slate-800'}`}>
                    <div className="flex flex-col min-w-0 pr-4">
                      <span className={`font-bold text-sm truncate ${plan.id === currentPlanId ? 'text-cyan-400' : plan.isLegacy ? 'text-amber-400' : 'text-slate-200'}`}>
                        {plan.name} {plan.id === currentPlanId && <span className="ml-2 text-[10px] bg-cyan-900/50 text-cyan-300 px-2 py-0.5 rounded-full uppercase">{t('event_manager.in_use', 'In Uso')}</span>}
                      </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {!plan.isLegacy && (
                        <button 
                          ...
                          className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-colors opacity-50 group-hover:opacity-100 border ${copiedLinkId === plan.id ? 'bg-emerald-900/50 text-emerald-400 border-emerald-700' : 'bg-slate-800 hover:bg-indigo-900 text-slate-500 hover:text-indigo-400 border-slate-700 hover:border-indigo-900'}`}
                          title={t('event_manager.copy_link', 'Copia Link Spettatore')}
                        >
                          {copiedLinkId === plan.id ? '✓' : '🔗'}
                        </button>
                      )}
                      ...
                      <button 
                        ...
                        title={t('event_manager.delete_btn', 'Elimina')}
                      >
                        ✕
                      </button>
                    </div>
          ...
          <div className="w-full md:w-[350px] flex flex-col gap-6 shrink-0">
            ...
            <div className="bg-emerald-950/20 border border-emerald-900/50 rounded-2xl flex flex-col shadow-inner overflow-hidden">
              ...
              <div className="p-4 flex flex-col gap-3">
                <label className="text-[10px] font-bold text-slate-400 uppercase">{t('event_manager.current_plan_name', 'Nome del Piano Attuale')}</label>
                <input 
                  type="text" 
                  value={newPlanName} 
                  ...
                  placeholder={t('event_manager.placeholder_name', 'Es. Strategia Finale')}
                />
                  ...
                  {currentPlanId ? (
                    <>
                      <button onClick={(e) => saveToFirebase(e, true)} disabled={!newPlanName.trim() || isSaving} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:bg-slate-700 text-white font-black py-2.5 rounded-xl transition-colors shadow-lg text-xs flex justify-center gap-2">
                        {isSaving ? t('event_manager.saving_short', '⏳...') : t('event_manager.update_plan', 'Aggiorna Piano Esistente')}
                      </button>
                      <button onClick={(e) => saveToFirebase(e, false)} disabled={!newPlanName.trim() || isSaving} className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:bg-slate-700 text-white font-black py-2.5 rounded-xl transition-colors shadow-lg text-xs flex justify-center gap-2">
                        {isSaving ? t('event_manager.saving_short', '⏳...') : t('event_manager.save_copy', 'Salva come Nuova Copia')}
                      </button>
                    </>
                  ) : (
                    <button onClick={(e) => saveToFirebase(e, false)} disabled={!newPlanName.trim() || isSaving} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:bg-slate-700 text-white font-black py-3 rounded-xl transition-colors shadow-lg text-xs flex justify-center gap-2">
                      {isSaving ? t('event_manager.saving', '⏳ Salvataggio...') : t('event_manager.save_new_cloud', 'Salva Nuovo nel Cloud')}
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="bg-amber-950/20 border border-amber-900/50 p-4 rounded-2xl flex flex-col gap-3 shadow-inner text-center items-center">
               <h3 className="text-xs font-black text-amber-500 uppercase tracking-widest">{t('event_manager.start_from_scratch', 'Ricomincia da Capo')}</h3>
               <p className="text-[10px] text-slate-400">{t('event_manager.scratch_desc', 'Azzera la mappa tattica e inizia un nuovo piano.')}</p>
               <button 
               ...
               >
                 {t('event_manager.clear_board', 'Azzera Lavagna Tattica')}
               </button>
            </div>

            <div className="bg-slate-900/50 border border-slate-700/50 p-4 rounded-2xl flex flex-col gap-3 shadow-inner">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">{t('event_manager.local_backup', 'Backup Locale')}</h3>
              <div className="flex gap-2">
                <button onClick={exportToFile} className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-300 font-bold py-2 rounded-lg transition-colors text-[10px] uppercase flex justify-center items-center gap-1">{t('event_manager.export', 'Esporta JSON')}</button>
                <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-300 font-bold py-2 rounded-lg transition-colors text-[10px] uppercase flex justify-center items-center gap-1">{t('event_manager.import', 'Importa JSON')}</button>
