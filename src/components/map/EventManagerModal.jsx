...
export default function EventManagerModal({ 
  ...
  // 💡 NUOVE PROPS PER RENDERLO UNIVERSALE
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      // 💡 RICERCA DEI VECCHI SALVATAGGI IN BASE ALLA MODALITÀ (Castello o Swordland)
      try {
        for (const legacyId of legacyIds) {
          ...
          if (legacySnap.exists()) {
            ...
            loadedPlans.push({
              id: legacySnap.id,
              name: "💾 Autosalvataggio Precedente (Legacy)",
              data: legacyData.version ? legacyData : { // Adatta se è Swordland o Castello
    ...
    return () => unsubscribe();
  }, [isOpen, allianceCode, dbCollection, legacyCollection, legacyIds]);
  ...
  useEffect(() => {
    if (isOpen) {
      if (currentPlanId && currentPlanName) {
        ...
        setNewPlanName(`Piano ${allianceCode || 'Regno'} - ${formattedDate}`);
      }
    }
  }, [isOpen, allianceCode, currentPlanId, currentPlanName]);
  ...
  const deletePlanFromFirebase = async (id) => {
    if (window.confirm('🗑️ Sei sicuro di voler eliminare questo piano? Nessun ufficiale potrà più accedervi.')) {
      try {
        ...
        if (id === currentPlanId) {
          onPlanSaved(null, '');
          setNewPlanName(`Nuovo Piano ${allianceCode}`);
        }
      } catch (error) {
  ...
  const importFromFile = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        alert('❌ Errore nella lettura del file. Potrebbe essere corrotto.');
      }
    };
  ...
  const formatDate = (timestamp) => {
    if (!timestamp || !timestamp.seconds) return 'Data sconosciuta';
        ...
        <div className="flex justify-between items-start mb-6 border-b border-slate-800 pb-4 shrink-0">
          <div>
            <h2 className="text-2xl font-black text-white flex items-center gap-3">
              <span>☁️</span> Database Piani Tattici
            </h2>
            <p className="text-sm text-slate-400 mt-1">Gestisci le simulazioni della tua Alleanza [{allianceCode}].</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col md:flex-row gap-6 pb-2">
          ...
          <div className="flex-1 bg-slate-950/50 border border-slate-800 rounded-2xl flex flex-col shadow-inner overflow-hidden">
            <div className="bg-slate-900 p-4 border-b border-slate-800 flex justify-between items-center shrink-0">
              ...
              <span className="text-xs text-slate-500 font-bold">{savedPlans.length} Piani</span>
            </div>
            
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-3 min-h-[300px]">
              {isFetching && savedPlans.length === 0 ? (
                <div className="text-center text-cyan-400 font-bold py-10 animate-pulse">Sincronizzazione col database in corso...</div>
              ) : savedPlans.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 py-10 animate-in fade-in duration-500">
                  ...
                  <p className="text-sm font-bold">Nessun piano salvato</p>
                  <p className="text-xs italic mt-1">Crea e salva il tuo primo piano per vederlo qui.</p>
                </div>
              ) : (
                savedPlans.map(plan => (
                  <div key={plan.id} className={`bg-slate-900 border p-3 rounded-xl flex items-center justify-between hover:border-cyan-500/50 transition-colors group ${plan.id === currentPlanId ? 'border-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'border-slate-800'}`}>
                    <div className="flex flex-col min-w-0 pr-4">
                      <span className={`font-bold text-sm truncate ${plan.id === currentPlanId ? 'text-cyan-400' : plan.isLegacy ? 'text-amber-400' : 'text-slate-200'}`}>
                        {plan.name} {plan.id === currentPlanId && <span className="ml-2 text-[10px] bg-cyan-900/50 text-cyan-300 px-2 py-0.5 rounded-full uppercase">In uso</span>}
                      </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      >
                        {plan.id === currentPlanId ? 'Attivo' : '▶ Carica'}
                      </button>
                      <button 
          ...
          <div className="w-full md:w-[350px] flex flex-col gap-6 shrink-0">
            ...
            <div className="bg-emerald-950/20 border border-emerald-900/50 rounded-2xl flex flex-col shadow-inner overflow-hidden">
              ...
              <div className="p-4 flex flex-col gap-3">
                <label className="text-[10px] font-bold text-slate-400 uppercase">Nome del Piano Attuale:</label>
                <input 
                  type="text" 
                  value={newPlanName} 
                  ...
                  {currentPlanId ? (
                    <>
                      <button onClick={(e) => saveToFirebase(e, true)} disabled={!newPlanName.trim() || isSaving} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:bg-slate-700 text-white font-black py-2.5 rounded-xl transition-colors shadow-lg text-xs flex justify-center gap-2">
                        {isSaving ? '⏳...' : '💾 Aggiorna questo Piano'}
                      </button>
                    ...
                    <button onClick={(e) => saveToFirebase(e, false)} disabled={!newPlanName.trim() || isSaving} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:bg-slate-700 text-white font-black py-3 rounded-xl transition-colors shadow-lg text-xs flex justify-center gap-2">
                      {isSaving ? '⏳ Salvataggio...' : '☁️ Salva Nuovo nel Cloud'}
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="bg-amber-950/20 border border-amber-900/50 p-4 rounded-2xl flex flex-col gap-3 shadow-inner text-center items-center">
               <h3 className="text-xs font-black text-amber-500 uppercase tracking-widest">✨ Inizia da Zero</h3>
               <p className="text-[10px] text-slate-400">Vuoi svuotare la mappa, rimuovere tutte le formazioni e ricominciare un nuovo piano pulito?</p>
               <button 
            ...
            <div className="bg-slate-900/50 border border-slate-700/50 p-4 rounded-2xl flex flex-col gap-3 shadow-inner">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">💽 Backup su PC Locale</h3>
              <div className="flex gap-2">
                <button onClick={exportToFile} className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-300 font-bold py-2 rounded-lg transition-colors text-[10px] uppercase flex justify-center items-center gap-1">📥 Esporta</button>
                <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-300 font-bold py-2 rounded-lg transition-colors text-[10px] uppercase flex justify-center items-center gap-1">📤 Importa</button>
